...
echo "Install oh my zsh for dev"
sudo -u dev sh -c "$(curl -fsSL https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh)"
echo "\ue0b0 \u00b1 \ue0a0 \u27a6 \u2718 \u26a1 \u2699"

# Set ZSH_THEME to agnoster for 'dev' user
if [ -f "$DEV_ZSHRC_FILE" ]; then
  echo "Setting ZSH_THEME to agnoster for dev..."
  sudo -u dev sed -i '' 's/^ZSH_THEME=.*/ZSH_THEME="agnoster"/' "$DEV_ZSHRC_FILE"
else
  echo "$DEV_ZSHRC_FILE does not exist."
fi
